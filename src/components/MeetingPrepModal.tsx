...
export function MeetingPrepModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl bg-[#141417] border border-[#2C2C30] shadow-2xl max-h-[80vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#2C2C30] shrink-0">
          <div>
        ...
        <div className="px-5 py-3 border-t border-[#2C2C30] shrink-0">
...
function PrepItem({ text, sub, urgent = false }: { text: string; sub?: string; urgent?: boolean }) {
  return (
    <div className={`px-3 py-2 rounded-xl border text-sm ${urgent ? 'bg-red-500/5 border-red-500/15' : 'bg-[#1C1C1F] border-[#2C2C30]'}`}>
