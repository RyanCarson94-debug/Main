...
export function TaskModal({ task, onSave, onClose }: TaskModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-[#1A1035] border border-[#2D1F5E] rounded-2xl shadow-2xl animate-[slideIn_0.2s_ease-out] overflow-hidden">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-black/30 border border-[#2D1F5E] rounded-xl px-4 py-3 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-purple-500 transition-colors"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full bg-black/30 border border-[#2D1F5E] rounded-xl px-4 py-3 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-purple-500 transition-colors resize-none"
            />
          </div>
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        ${selected ? `${q.bg} ${q.border} ${q.color}` : 'border-[#2D1F5E] text-gray-500 hover:border-gray-500'}
                      `}
                    >
                      {q.label}
                    </button>
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        ${selected ? `bg-white/10 border-white/20 ${p.color}` : 'border-[#2D1F5E] text-gray-500 hover:border-gray-500'}
                      `}
                    >
                ...
                <button
                  ...
                  className={`
                    ...
                    ${form.column === col
                      ...
                      : 'border-[#2D1F5E] text-gray-500 hover:border-gray-500'
                    }
                  `}
                >
                  {col.replace('-', ' ')}
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Add tag..."
                className="flex-1 bg-black/30 border border-[#2D1F5E] rounded-xl px-3 py-2 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-purple-500 transition-colors"
              />
              <button type="button" onClick={addTag} className="px-3 py-2 rounded-xl bg-white/5 border border-[#2D1F5E] text-gray-400 hover:text-white hover:border-gray-500 transition-colors">
                <Plus size={14} />
              </button>
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-3 rounded-xl border border-[#2D1F5E] text-gray-400 hover:text-white hover:border-gray-500 font-semibold text-sm transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="flex-1 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold text-sm hover:from-purple-500 hover:to-pink-500 transition-all hover:shadow-lg hover:shadow-purple-500/25"
            >
