          ...
          <div className="flex gap-3 items-end">
            <div className="flex-1 space-y-2">
              <input
                ...
                className="w-full bg-black/30 border border-[#2D1F5E] rounded-xl px-4 py-3 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-purple-500"
              />
              <input
                value={newQuarter}
                onChange={e => setNewQuarter(e.target.value)}
                placeholder="Quarter (e.g. Q2 2025)"
                className="w-full bg-black/30 border border-[#2D1F5E] rounded-xl px-4 py-2.5 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-purple-500"
              />
            </div>
      ...
      {okrs.length === 0 && !showForm ? (
        ...
        <div className="space-y-4 overflow-y-auto flex-1">
          {okrs.map(okr => {
            ...
            return (
              <div key={okr.id} className="rounded-2xl border border-[#2D1F5E] bg-white/2 overflow-hidden">
                {/* Objective header */}
                <div className="flex items-start justify-between p-4 border-b border-[#2D1F5E]">
                ...
                <div className="p-3 space-y-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 rounded-xl border border-dashed border-[#2D1F5E] text-gray-600 hover:text-gray-400 hover:border-gray-600 text-xs font-medium transition-colors w-full"
                  >
                    <Plus size={11} />
                    Add key result
                  </button>
